...
int main(int argc, char **argv) {
    ...
    std::map<double, Eigen::Matrix<double, 17, 1>> gt1_states_file;
    std::map<double, Eigen::Matrix<double, 17, 1>> gt2_states_file;
    ...
    if (nh->hasParam("path_gt1") && nh->hasParam("path_gt2"))
    {
        ...
        if (!path_to_gt.empty()) {
            ov_core::DatasetReader::load_gt_file(path_to_gt, gt1_states_file);
            PRINT_DEBUG("gt file path is: %s\n", path_to_gt.c_str());
        }
        nh->param<std::string>("path_gt2", path_to_gt, "");
        std::cout << path_to_gt << std::endl;
        if (!path_to_gt.empty()) {
            ov_core::DatasetReader::load_gt_file(path_to_gt, gt2_states_file);
            PRINT_DEBUG("gt file path is: %s\n", path_to_gt.c_str());
        }
    }
    ...
    for (auto const& x : gt1_states_file)
    {
        double t = x.first;
        ...
        gt1_states.push_back(msg);
    }

    for (auto const& x : gt2_states_file)
    {
        double t = x.first;
        geometry_msgs::PoseStamped msg;
        msg.header.stamp = ros::Time().fromSec(t); 
    ...
    // R_ItoC << 0.999903,   0.0138569, -0.00162008,
    //         -0.0138701,    0.999868, -0.00845159,
    //         0.00150276,  0.00847324,    0.999963;
    // Eigen::Vector3d p_IinC;
    // p_IinC << 0.00461925, -0.00339613,  -0.0350073;

    // // B I from vicon2gt
    // Eigen::Matrix3d R_BtoI;
    // R_BtoI << -0.0897974,  -0.995819,   -0.01677,
    //         -0.0746901,  0.0235238,  -0.996929,
    //         0.993155, -0.0882691, -0.0764902;
    // Eigen::Vector3d p_BinI;
    // p_BinI << -0.0650382, -0.0351697, -0.0710834;

    // Eigen::Matrix3d R_BtoC = R_ItoC * R_BtoI;
    // Eigen::Vector3d p_BinC = p_IinC + R_ItoC * p_BinI;
    ...
    // Eigen::Matrix3d R_ItoB;
    // R_ItoB << -0.0567861, -0.0541126,   0.996919,
    //             -0.998126,  0.0258827, -0.0554499,
    //             -0.0228024,  -0.998199, -0.0554809;
    // Eigen::Vector3d p_BinI;
    // p_BinI << 0.037731, -0.017854, -0.157857;
    ...
    //             Eigen::Matrix<double, 3, 1> t_B2_I1 = p_BinI + R_ItoB.transpose() * t_B2_B1;
    //             Eigen::Matrix<double, 3, 1> t_B2_W = t_I1_W + R_I1_W.transpose() * t_B2_I1;

    //             double time_stamp = t2;
    //             outFile_pose << std::fixed << std::setprecision(6) << time_stamp << " "
    //                             << t_B2_W(0) << " " << t_B2_W(1) << " " << t_B2_W(2) << " "
    //                             << std::endl;
