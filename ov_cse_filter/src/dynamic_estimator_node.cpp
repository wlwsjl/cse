...
int main(int argc, char *argv[]) {
  ...
  // estimator.onInit(nh);
  estimator.Init();
  estimator.pub_pathimu = nh.advertise<nav_msgs::Path>("/ov_msckf/pathimu", 1000);
...
#if (Online_Sub)
  // estimator.sub_input_  = nh.subscribe("input",  10,&DynamicEstimator::input_callback,  &estimator);
  // estimator.sub_odom_ = nh.subscribe("pose", 10,&DynamicEstimator::pose_callback, &estimator);
  // estimator.sub_imu_  = nh.subscribe("imu",  10,&DynamicEstimator::imu_callback,  &estimator);

  // estimator.pub_estimates_ = nh.advertise<dynamic_estimator::ParameterEstimates>("param_estimates", 10);
  ...
  while (ros::ok()) {
    ...
    if (should_process_pose) {
      ...
      // q_I1_W.x() = msg_pose_current->pose.pose.orientation.x; // quat
      // q_I1_W.y() = msg_pose_current->pose.pose.orientation.y;
      // q_I1_W.z() = msg_pose_current->pose.pose.orientation.z;
      // q_I1_W.w() = msg_pose_current->pose.pose.orientation.w;
