...
///                ROS_BREAK();
...
#if 0
            jacobian_attitude_i.block<3, 3>(O_R, O_R-O_R) = -(Qj.inverse() * Qi).toRotationMatrix();
#else
            ...
            if (jacobians[3])
            {
                Eigen::Map<Eigen::Matrix<double, 15, 3, Eigen::RowMajor>> jacobian_position_j(jacobians[3]);
            ...
            if (jacobians[4])
            {
                Eigen::Map<Eigen::Matrix<double, 15, 4, Eigen::RowMajor>> jacobian_attitude_j(jacobians[4]);
